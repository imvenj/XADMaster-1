...
-(instancetype)initWithFile:(NSString *)file error:(NSError **)error { return [self initWithFile:file delegate:nil error:error]; }
...
-(instancetype)initWithData:(NSData *)data error:(NSError **)error { return [self initWithData:data delegate:nil error:error]; }
...
-(instancetype)initWithArchive:(XADArchive *)otherarchive entry:(NSInteger)n error:(NSError **)error { return [self initWithArchive:otherarchive entry:n delegate:nil error:error]; }

-(instancetype)initWithArchive:(XADArchive *)otherarchive entry:(NSInteger)n delegate:(id)del error:(NSError **)error
{
	if((self=[self init]))
	{
     ...
     immediateExtractionTo:(NSString *)destination subArchives:(BOOL)sub error:(NSError **)error
{
	if((self=[self init]))
	{
		...
		else if(error) *error=[NSError errorWithDomain:XADErrorDomain code:XADSubArchiveError userInfo:nil];
...
-(BOOL)_parseWithErrorPointer:(NSError **)error
{
	...
	if(parseerror)
	{
		...
		if(error) *error=[NSError errorWithDomain:XADErrorDomain code:parseerror userInfo:nil];
	}

	if(immediatefailed&&error) *error=[NSError errorWithDomain:XADErrorDomain code:lasterror userInfo:nil];
...
-(CSHandle *)handleForEntry:(NSInteger)n error:(NSError **)error
{
	...
	{
		lasterror=[XADException parseException:e];
		if(error) *error=[NSError errorWithDomain:XADErrorDomain code:lasterror userInfo:nil];
	}
	return nil;
}
...
-(BOOL)extractEntries:(NSIndexSet *)entryset to:(NSString *)destination subArchives:(BOOL)sub
{
	...
	totalsize+=[self representativeSizeOfEntry:(NSInteger)i];
	...
	for(NSUInteger i=[entryset firstIndex];i!=NSNotFound;i=[entryset indexGreaterThanIndex:i])
	{
		...
		if(sub&&[self entryIsArchive:(NSInteger)i])
		{
			@try { res=[self extractArchiveEntry:(NSInteger)i to:destination]; }
			@catch(id e) { res=NO; }

			if(!res&&lasterror==XADDataFormatError) // Retry as regular file if the archive format was not known
			{
				res=[self extractEntry:(NSInteger)i to:destination deferDirectories:YES];
			}
		}
		else res=[self extractEntry:(NSInteger)i to:destination deferDirectories:YES];
		...
		extractsize+=[self representativeSizeOfEntry:(NSInteger)i];
...
-(BOOL)extractArchiveEntry:(NSInteger)n to:(NSString *)destination
{
	...
	for(;;)
	{
		...
		immediateExtractionTo:path subArchives:YES error:&err2];
		err = (XADError)err2.code;
...
-(int)sizeOfEntry:(NSInteger)n // deprecated and broken
{
	NSDictionary *dict=[self dataForkParserDictionaryForEntry:n];
	if(!dict) return 0; // Special case for resource forks without data forks
	NSNumber *size=dict[XADFileSizeKey];
...
-(void *)xadFileInfoForEntry:(NSInteger)n
{
	struct xadFileInfo
	{
		void *xfi_Next;
...
-(BOOL)extractEntry:(NSInteger)n to:(NSString *)destination overrideWritePermissions:(BOOL)override
{ return [self extractEntry:n to:destination deferDirectories:override resourceFork:YES]; }

-(BOOL)extractEntry:(NSInteger)n to:(NSString *)destination overrideWritePermissions:(BOOL)override resourceFork:(BOOL)resfork
...
-(XADAction)archive:(XADArchive *)archive entry:(NSInteger)n collidesWithFile:(NSString *)file newFilename:(NSString **)newname { return XADOverwriteAction; }
-(XADAction)archive:(XADArchive *)archive entry:(NSInteger)n collidesWithDirectory:(NSString *)file newFilename:(NSString **)newname { return XADSkipAction; }
-(XADAction)archive:(XADArchive *)archive creatingDirectoryDidFailForEntry:(NSInteger)n { return XADAbortAction; }
...
-(void)archive:(XADArchive *)archive extractionProgressForEntry:(NSInteger)n bytes:(off_t)bytes of:(off_t)total {}
-(void)archive:(XADArchive *)archive extractionOfEntryDidSucceed:(NSInteger)n {}
-(XADAction)archive:(XADArchive *)archive extractionOfEntryDidFail:(NSInteger)n error:(XADError)error { return XADAbortAction; }
...
-(XADAction)archive:(XADArchive *)archive nameDecodingDidFailForEntry:(NSInteger)n bytes:(const char *)bytes { return XADAbortAction; }
