      ...
      do
      {
        ...
        if(!e)
        {
          ...
          case ST_LINKFILE: case ST_LINKDIR:
            ...
            break;
          case ST_SOFTLINK:
            j = (xadUINT32)strlen((const char *)fh->afs_SoftLinkName)+1;
            ...
            if(!(err = parsedir(&pd))) /* parse root block */
            {
              ...
              while(!err && pd.LinkList)
              {
                ...
                if(ofi)
                {
                  ...
                  j = (xadUINT32)strlen((const char *)ofi->xfi_FileName)+1;
                  k = par ? (xadUINT32)strlen((const char *)par->xfi_FileName)+1 : 0;
