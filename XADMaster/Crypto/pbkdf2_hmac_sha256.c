...
{
	int numblocks=(unsigned int)((DKlength+HMAC_SHA256_DIGEST_LENGTH-1)/HMAC_SHA256_DIGEST_LENGTH);
	for(int i=0;i<numblocks;i++)
	{
		...
		HMAC_SHA256_UpdateKey(&ctx,password,(unsigned int)passwordlength);
		HMAC_SHA256_EndKey(&ctx);
		HMAC_SHA256_StartMessage(&ctx);
		HMAC_SHA256_UpdateMessage(&ctx,salt,(unsigned int)saltlength);
...
{
	int numblocks=(int)((DKlength+HMAC_SHA256_DIGEST_LENGTH-1)/HMAC_SHA256_DIGEST_LENGTH);
	for(int i=0;i<numblocks;i++)
	{
		uint8_t Uj[HMAC_SHA256_DIGEST_LENGTH];
